...
;; `defrecord`, not `deftype`: SCI/nbb cannot resolve custom implementations of
;; ClojureScript's built-in IEquiv/IHash protocols on a deftype. A record has
;; structural equality and hashing generated by each runtime, while ILink keeps
;; callers independent of the fact that a record is map-like. This is the same
;; portability rule used by cbor.core/Tagged and OrderedMap.
(defrecord Link [cid]
  ILink
  (-link-cid [_] cid))
